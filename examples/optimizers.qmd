...
The built-in `Logit` class reports the linear index and probability for label 1. The synthetic data below are therefore generated from

`Pr(y = 1 | x) = sigmoid(theta_0 + x @ theta)`.
...
prob_one = sigmoid(X @ theta_true)
y = rng.binomial(1, prob_one).astype(np.int32)
...
```{python}
prob_one_bfgs = sigmoid(X @ bfgs_result["x"])
prob_one_logit = sigmoid(X @ logit_theta)
...
axes[1].scatter(prob_one_bfgs, prob_one_logit, s=12, alpha=0.55)
axes[1].plot([0, 1], [0, 1], color="black", linewidth=1.0)
axes[1].set_title("Class-1 fitted probabilities")
