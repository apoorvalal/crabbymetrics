...
def simulate_one(seed, outcome_correct, pscore_correct):
    ...
    native = cm.AIPW(penalty=np.logspace(-3, 1, 8), cv=2, n_folds=2, seed=seed)
...
native = cm.AIPW(penalty=np.logspace(-3, 1, 8), cv=2, n_folds=2, seed=12)
native.fit(y, d, x)
...
The chapter's basic lesson survives the translation: doubly robust scores are far harder to break than pure regression adjustment or pure weighting, and the current library's `AIPW` class is the high-level version of that same logic.
