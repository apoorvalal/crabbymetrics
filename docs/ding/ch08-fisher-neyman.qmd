...
def fisher_pvalue(y_obs, z_obs, rng, n_frt=2000):
    tau_hat = diff_in_means(y_obs, z_obs)
    null_stats = np.zeros(n_frt)
    for draw in range(n_frt):
        z_null = np.zeros(n)
        z_null[rng.choice(n, size=n1, replace=False)] = 1.0
        null_stats[draw] = diff_in_means(y_obs, z_null)
...
for _ in range(80):
    ...
    fisher_draws.append(fisher_pvalue(y_obs, z, rng))
