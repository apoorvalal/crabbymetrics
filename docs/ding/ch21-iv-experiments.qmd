...
## JOBS One-Sided Noncompliance Example
...
```{python}
jobs = pd.read_csv(repo_root() / "ding_w_source" / "repl" / "jobsdata.csv")
z_jobs = jobs["treat"].to_numpy(dtype=float)
d_jobs = jobs["comply"].to_numpy(dtype=float)
y_jobs = jobs["job_seek"].to_numpy(dtype=float)

job_controls = ["sex", "age", "marital", "nonwhite", "educ", "income"]
x_jobs = pd.get_dummies(jobs[job_controls], drop_first=True, dtype=float).to_numpy(dtype=float)
x_jobs = (x_jobs - x_jobs.mean(axis=0)) / np.where(x_jobs.std(axis=0) > 1e-12, x_jobs.std(axis=0), 1.0)

jobs_iv = cm.TwoSLS()
jobs_iv.fit(d_jobs[:, None], x_jobs, z_jobs[:, None], y_jobs)
jobs_iv_hc1 = jobs_iv.summary(vcov="hc1")

pd.DataFrame(
    {
        "estimate": [
            wald(z_jobs, d_jobs, y_jobs),
            float(jobs_iv_hc1["coef"][0]),
        ],
        "se": [
            wald_delta_se(z_jobs, d_jobs, y_jobs),
            float(jobs_iv_hc1["coef_se"][0]),
        ],
    },
    index=["Wald", "Covariate-adjusted TwoSLS"],
)
```
