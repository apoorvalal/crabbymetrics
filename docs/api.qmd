...
- `HorizontalPanelRidge.fit(y, treated_units, t_pre)` expects a balanced outcome panel in an `(n_units, n_periods)` matrix, a sequence of treated unit indices, and the number of pre-treatment periods. It uses donor paths as features, trains on pre-treatment periods, and predicts the treated counterfactual path.
...
- `SyntheticDID.fit(y, treated_units, t_pre)` expects a balanced outcome panel in an `(n_units, n_periods)` matrix, a sequence of treated unit indices, and the number of pre-treatment periods.
- `MatrixCompletion.fit(y, mask=None)` implements MCPanel-style nuclear-norm panel completion with optional additive unit/time effects; missing entries can be passed through `mask` or as non-finite values when `mask=None`.
- `InteractiveFixedEffects.fit(y)` implements the no-covariate fect principal-components IFE path for balanced panels; `panel_factor()` and `panel_fe()` expose the lower-level fect SVD helpers.
...
reference_rows = [
    ...
    [
        ...
        "1D float treated counterfactual path",
        "<code>att</code>, <code>intercept</code>, <code>coef</code>, <code>counterfactual</code>, <code>treated_outcome</code>, <code>treatment_effect</code>, <code>pre_rmse</code>, <code>penalty</code>, <code>control_units</code>, <code>treated_units</code>, <code>t_pre</code>",
        "No bootstrap method",
        "Horizontal panel regression: train a ridge forecast of the treated pre-period average outcome using donor outcomes as contemporaneous features, then predict the treated post-treatment counterfactual from donor post-treatment paths.",
    ],
    [
