...
`crabbymetrics` is a compact Rust-backed econometrics library exposed to Python through `pyo3` and packaged with `maturin`. The public API is intentionally small: ten estimator classes, `numpy` arrays as the only runtime dependency, and a scikit-adjacent `fit` / `predict` / `summary` / `bootstrap` pattern.
...
- Coverage is broad for a small codebase: OLS, fixed-effects OLS, synthetic control, ElasticNet, binary and multinomial logit, Poisson, TwoSLS, FTRL, and a callback-driven general `MEstimator`.
