...
Moment and Jacobian arrays must be finite with stable shapes. The solver scales
its linear systems and checks undamped first-order and Newton-step residuals;
a small damped step or absolute objective change alone does not mean convergence.
`ridge` is dimensionless solver damping in diagonally normalized coordinates,
not a statistical penalty: two-step weights and reported covariance use undamped
inverses. Rank-deficient information is rejected.

Fitted moments and their Jacobian are snapshotted at fit time. Subsequent
`summary()` and `wald_test()` calls do not re-evaluate mutable Python callbacks.
Failed refits clear the fitted state. Default sandwich inference is unchanged.
`vcov="vanilla"` requires two-step iid weighting, or the explicit keyword
`assume_optimal_weighting=True` asserting the information-identity assumption.
The latter is an assumption, not a correction for a nonoptimal W. `j_test_valid`
and `j_test_assumptions` distinguish a raw criterion statistic from its intended
asymptotic iid optimal-weight interpretation; they cannot verify moment validity.
...
vanilla = lin.summary(vcov="vanilla", assume_optimal_weighting=True)
