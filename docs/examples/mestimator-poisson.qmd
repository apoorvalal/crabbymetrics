...
`fit()` now computes and stores the score-based covariance before returning,
using the fitted score Jacobian. Later mutation of the callback data cannot
change `summary()`. Invalid scores or failed covariance construction reject the
fit and clear its state. The original callback data is still retained for
bootstrap resampling and must remain unchanged when using `bootstrap()`.
