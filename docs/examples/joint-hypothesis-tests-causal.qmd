...
The same idea applies to staggered-adoption panels. A conventional two-way fixed-effect event study pools dynamic treatment effects across adoption cohorts:
...
def event_study_design(df, rel_min=-2, rel_max=3, baseline=-1):
    ...
    parts.extend([unit_dummies.to_numpy(), year_dummies.to_numpy(), work[["x"]].to_numpy(float)])
    names.extend(list(unit_dummies.columns) + list(year_dummies.columns) + ["x"])
    ...
    return xmat, names, deviation_names


def fit_event_case(heterogeneous, seed):
    df = make_panel(heterogeneous=heterogeneous, seed=seed)
    xmat, names, deviation_names = event_study_design(df)
    model = cm.OLS()
    model.fit(xmat, df["y"].to_numpy())

    # OLS coefficient order is [intercept, names...]. Restrict all cohort
    # deviation coefficients to zero.
    name_to_pos = {name: j + 1 for j, name in enumerate(names)}
    r = np.zeros((len(deviation_names), len(names) + 1))
