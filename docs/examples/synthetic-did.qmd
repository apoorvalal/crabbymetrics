...
sdid.fit(observed_panel, treated_units.tolist(), t_pre)
...
```{python}
unit_weights = np.asarray(sdid_summary["unit_weights"])
time_weights = np.asarray(sdid_summary["time_weights"])
...
axes[0].plot(
    time + 1,
    np.asarray(sdid_summary["synthetic_outcome"]),
    color="#7570b3",
    lw=2.0,
...
axes[1].plot(
    event_time,
    np.asarray(sdid_summary["treatment_effect"]),
    color="#7570b3",
    lw=2.0,
