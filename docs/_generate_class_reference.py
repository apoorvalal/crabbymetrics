#!/usr/bin/env python3
"""Generate first-pass class-centered Quarto reference pages."""
...
The user supplies a Python callback returning the per-observation moment matrix. In exactly identified cases the class can solve by Gauss-Newton; in overidentified cases it can use identity or two-step weighting and report sandwich covariance estimates.""",
 api="""Construct with `GMM(moment_fn, jacobian_fn=None, max_iterations=100, tolerance=1e-6, ridge=1e-8, fd_eps=1e-6)`. `fit(data, theta0, weighting='auto')` stores the fitted parameters. `fit_sketch(...)` row-sketches the moment problem. `summary(vcov='sandwich', omega='iid', lags=None, clusters=None)` controls inference.""",
...
for class_name, meta in PAGES.items():
    ...
    (REF / f"{class_name}.qmd").write_text(restore_latex_escapes(text))

# API landing page
api = """---
title: "API"
subtitle: "Class-named pages grouped by modeling task"
format:
  html:
    toc: true
    toc-depth: 3
...
- [PCA](reference/PCA.qmd)
- [KernelBasis](reference/KernelBasis.qmd)

## Estimation interfaces and utilities
...
"""
(ROOT / "api.qmd").write_text(api)
print(f"Wrote {len(PAGES)} class pages under {REF}")
