...
- **horizontal ridge** is now the first-class `crabbymetrics.HorizontalPanelRidge`: it regresses the treated pre-treatment time series on donor outcomes over time, then forecasts treated post outcomes from donor post outcomes;
...
Synthetic control, synthetic DID, and matrix completion are also first-class `crabbymetrics` panel estimators.
...
def estimate_horizontal_ridge(panel, treated_units, t_pre, alpha=0.25):
    ...
    return Estimate(
        "Horizontal ridge",
        float(summary["att"]),
        np.asarray(summary["treatment_effect"])[t_pre:],
        np.asarray(summary["counterfactual"]),
