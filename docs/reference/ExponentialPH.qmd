...
1. `fit()` clears fitted state, validates finite $X$, positive finite times, binary floating-point event indicators, and the solver controls. It initializes $(\alpha,\beta')$ at $(\log\{\max(\sum d_i,1)/\max(\sum t_i,10^{-12})\},0')$; the event count is floored at one, so an all-censored sample still receives a finite starting hazard.
