...
1. `fit()` clears fitted state, validates the initial vector and controls, and stores Python callable objects inside an Argmin problem. Each cost request calls `objective_fn(theta, data)`, requires a two-element tuple, and extracts only its scalar first element.
2. Each gradient request calls the same Python function again and extracts the second tuple element as a one-dimensional NumPy array. Cost and gradient remain separate calls. Objectives and gradients must be finite, and gradient length must equal the parameter count.
3. Seven-pair L-BFGS starts exactly at `theta0`, uses More-Thuente line search, and applies the same tolerance to gradient and cost stopping. The wrapper converts Argmin termination to common diagnostics and rejects an iteration cap or other nonconverged status before storing parameters and data.
4. Covariance is computed before `fit()` succeeds. The score callback must return a finite nonempty $n\times k$ matrix with $k=\dim\theta$; each coordinate perturbation must retain that shape. Invalid score inference rejects the fit and clears state.
5. Column means of those perturbed score matrices yield the central-difference bread $A$. The meat is the uncentered $\Psi'\Psi/n$. The code explicitly inverts $A$, computes $A^{-1}BA^{-T}/n$, and symmetrizes it. Summaries reuse this fit-time covariance, so later callback-data mutation does not silently change inference. An explicit `compute_vcov()` call recomputes it from the retained data. Bootstrap still retains the original Python data: keep that data immutable if using bootstrap.
6. Bootstrap reads `data['n']`, draws row indices, shallow-copies the Python dictionary, inserts an `indices` array, and calls the same L-BFGS problem from the original fitted estimate. The score callback is not used in bootstrap fitting. Correct resampling therefore depends entirely on `objective_fn` honoring that injected key.
