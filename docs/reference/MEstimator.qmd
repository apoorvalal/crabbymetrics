...
5. Column means of those perturbed score matrices yield the central-difference bread $A$. The meat is the uncentered $\Psi'\Psi/n$. The code explicitly inverts $A$, computes $A^{-1}BA^{-T}/n$, and symmetrizes it. All summaries reuse this fit-time covariance, so later callback-data mutation cannot change inference. Bootstrap still retains the original Python data: keep that data immutable if using bootstrap.
