...
- `DynamicCovariateBalance(nuisance_penalty, autoscale, max_weight, max_iterations, tolerance)` implements Viviano and Bradic's full-interaction recursive potential projections and augmented treatment-path mean. At period $t$, units following the target prefix are calibrated to the previous prefix's weighted history through the same bounded quadratic engine as `BalancingWeights`. The first implementation uses ridge and exact balance, retains explicit per-period solver status, and does not claim analytic inference.

Tests use three causal DGPs rather than only API smoke checks. The regression-blip DGP contains a time-varying covariate affected by lagged treatment and recovers the total mediated lag response. The parallel-trends DGP contains unobserved confounding of treatment and outcome levels but not untreated trends and recovers a three-horizon staggered-initiation response. The dynamic-balancing DGP verifies recursive path support, sequential weights, exact mean balance, and a known two-period path contrast. The review page derives all three estimators and includes a 100-replication comparison under observed selection and hidden time-invariant level confounding, with Mermaid DAGs and Matplotlib sampling-distribution and bias/variance figures.
...
- Ran a separate 200-replication R/Python comparison for the complete 398-cell slide grid. Of those cells, 390 are within three combined Monte Carlo standard errors. All FE, IFE, generalized synthetic-control, and panel-VAR cells pass this rule. The eight exceptions are five correlated MCPanel cells in the poor-overlap classic-factor design and three ARIMA cells in the low-persistence time-series design.
- Ran external fixed-panel parity checks for FE, IFE, MCPanel, generalized synthetic-control, and panel-VAR outcome surfaces. These larger reproduction artifacts are not committed in this repository. A canonical poor-overlap MCPanel diagnostic limits the maximum treated-post R/Python surface difference to $0.0096$, which separates the remaining Monte Carlo variation from the balancing-estimator implementation.
- Added committed algebra, recovery, target-mapping, validation, and 48-case R-parity tests plus a public class reference page. Separate external ranking checks require double balancing to improve on unit-only balancing in the strong-factor, weak-factor, mixed, and persistent time-series cases where the R experiments show that result.
...
## 2026-08-20 Augmented Balancing Documentation

- Expanded `docs/reference/AugmentedBalancing.qmd` into a contract-first API page covering the input/treatment contract, estimator configurations, methods, errors, computational scaling, weight-row mappings, and every `summary()` field.
...
- Created `speedtest` from `origin/master` and inventoried all 30 estimator classes exported by Crabbymetrics.
