...
def simulate_gmm(sample_sizes, reps, seed):
    ...
    for n in sample_sizes:
        ...
        for _ in range(reps):
            ...
            # Deliberately invalid covariance, retained only as a negative control.
            jacobian = gmm_jacobian(sandwich["coef"], {"x": x, "z": z})
            naive_variance = 1.0 / (n * float((jacobian.T @ jacobian)[0, 0]))
            slope_estimates.append(float(sandwich["coef"][0]))
            vanilla_ses.append(float(np.sqrt(naive_variance)))
            sandwich_ses.append(float(sandwich["se"][0]))

        out.append({"n": n, "method": "Naive identity (invalid)", **summarize_method(slope_estimates, vanilla_ses, beta_true)})
...
## GMM: Invalid Identity Shortcut Versus Sandwich
...
The native estimator supplies the sandwich column. Version 0.9 rejects the
vanilla shortcut for this identity-weighted fit unless the caller asserts
optimal weighting, which is false here. The invalid column is therefore an
explicit NumPy reference calculation, not a recommended API call. Its purpose
is to show the coverage failure prevented by that new guard.
